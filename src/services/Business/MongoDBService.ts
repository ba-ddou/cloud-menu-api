...
export default class MongoDBService {
	async getBusiness(
		...
		if (source == "QR") this.incrementBusinessQrCodeScanCount(id);
		if (document) return document;
	}
	async incrementBusinessQrCodeScanCount(id: string) {
		await BusinessModel.updateOne(
			{ _id: id },
			{ $inc: { "metrics.qrCodeScanCount": 1 } },
		);
	}
	...
	async assertBusinessAndSection(
		businessId: string,
		sectionId: string
	): Promise<BusinessDocument | null> {
		let document = await BusinessModel.findById(businessId);
